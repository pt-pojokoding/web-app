...
import buttonSfx from '/sounds/sound-1.mp3'
...
function checkAnswer() {
  ...
  if (quiz[currentQuestionIndex.value].choices[selectedChoiceIndex.value].isCorrect) {
...
const { play: playSoundBenar } = useSound(buttonSfx)
...
<template>
  <div id="animasi_benar" class="fixed bottom-0 right-4 translate-x-[100rem]">
    <div class="w-[20rem] aspect-square relative">
      <img src="/images/karakter-1.png" alt="" class="">
      <div class="bg-slate-800 absolute top-8 left-[-88%] px-6 py-2 rounded-md">
        <p class="text-white">Kamu Berhasil menyelesaikan kuis👍👍</p>
      </div>
    </div>
  </div>
  ...
  <UCard data-cy="quiz">
    <div v-if="authStore.user">
      ...
      <div v-if="!progressStore.getCurrentContentProgress(contentStore.currentContent._id) && !quizStarted"
           class="flex flex-col gap-4" data-cy="quiz-intro">
        ...
        <div v-for="(question, questionIndex) in quiz" :key="questionIndex" :data-cy="'quiz-question-' + questionIndex">
          <div v-if="questionIndex === currentQuestionIndex" data-cy="quiz-current-question">
            ...
            <ul class="flex flex-col gap-3" data-cy="quiz-current-question-choices">
              <UCard v-if="showExplanation && userAnswerIsCorrect" class="border border-green-400"
                     data-cy="quiz-current-question-correct-explanation">
                <p>
                  {{ selectedChoiceExplanation }}
                </p>
              </UCard>
              <UCard v-if="showExplanation && !userAnswerIsCorrect" class="border border-red-400"
              ...
              <li v-for="(choice, choiceIndex) in question.choices" :key="choiceIndex">
                <label :data-cy="'quiz-current-question-choice-' + choiceIndex">
                  <!-- <URadio
                                        ...
                                        @input="selectedChoiceIndex = choiceIndex; console.log('choice.id', choice._key)"
                                        :ui="{
                                            wrapper: 'p-5 border border-slate-600 rounded-lg cursor-pointer',
                                        }"
                                    ></URadio> -->
                  <URadio :label="choice.text" :value="choice._key" v-model="isSelectedAnswerCorrect"
                          @input="selectedChoiceIndex = choiceIndex; console.log('choice.id', choice)" :ui="{
                            wrapper: 'p-5 border border-slate-600 rounded-lg cursor-pointer',
                          }"></URadio>
                </label>
              </li>
              <UButton v-if="isSelectedAnswerCorrect !== null && !userAnswerIsCorrect" @click="checkAnswer"
                       class="self-end" data-cy="quiz-check-answer-button">Cek Jawaban</UButton>
              <UButton v-if="userAnswerIsCorrect" @click="nextQuestion" class="self-end"
                       data-cy="quiz-next-question-button">
                <p v-if="currentQuestionIndex === quiz.length - 1" @click="QuizSelesai">Selesai</p>
